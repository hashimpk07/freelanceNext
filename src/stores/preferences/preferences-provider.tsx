...
export const PreferencesStoreProvider = ({
  ...
  useEffect(() => {
    ...
    const domSidebarCollapsibleAttr = root.getAttribute(
      "data-sidebar-collapsible",
    );

    const safePreset = getSafeValue<PreferencesState["themePreset"]>(
      domPresetAttr,
      THEME_PRESET_VALUES,
    );
    ...
    const safeNavbarStyle = getSafeValue<PreferencesState["navbarStyle"]>(
      domNavbarStyleAttr,
      NAVBAR_STYLE_VALUES,
    );
    ...
    const safeSidebarCollapsible = getSafeValue<
      PreferencesState["sidebarCollapsible"]
    >(domSidebarCollapsibleAttr, SIDEBAR_COLLAPSIBLE_VALUES);
